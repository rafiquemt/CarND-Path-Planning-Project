...
void rotateSinglePoint(double &x, double& y, double ref_x, double ref_y, double ref_yaw) {
...
void rotatePoints(vector<double>& ptsx, vector<double>& ptsy, 
  double ref_x, double ref_y, double ref_yaw) {
    for (int i = 0; i < ptsx.size(); i++) {
      rotateSinglePoint(ptsx[i], ptsy[i], ref_x, ref_y, ref_yaw);
    }
}
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // New actuator coordinates (appended after any prev points)
          vector<double> ptsx;
          vector<double> ptsy;
          ...
          if (prev_size != previous_path_y.size()) {
            cout << "previous path size mismatch. Exiting!";
            exit(-1);
          }
          const double deltat_per_point = 0.020;
          double v_max = 49.5;
          ...
          if (prev_size > 0) {
            car_s = end_path_s; // predicting that the car will be at end_path_s
          }
          ...
          // generate spaced out points in map coordinates
          auto wp_d = 2 + 4 * lane;
